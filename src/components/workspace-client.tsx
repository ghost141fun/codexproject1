...
import {
  ...
  Download, Trash2, ExternalLink, Blocks, Hash, Pin,
} from 'lucide-react';
...
export function WorkspaceClient({ user, channels: initialChannels, directMessages, files: initialFiles }: WorkspaceClientProps) {
  ...
  const renderSidebar = () => {
    switch (activeView) {
      case 'dms': return <DmSidebar directMessages={directMessages} activeDm={activeDm} setActiveDm={handleSelectDm} onSelect={handleSelectChannel} />;
      case 'files': return <FilesSidebar activeCategory={activeCategory} onCategorySelect={setActiveCategory} fileCount={files.length} onUpload={handleUpload} searchQuery={searchQuery} onSearchChange={setSearchQuery} />;
      case 'huddles': return <HuddlesSidebar channels={channels} activeHuddles={activeHuddles} onStart={handleStartHuddle} />;
      ...
      case 'integrations': return (
        <div className="w-[260px] h-full flex flex-col bg-[#19171d] border-r border-white/[0.07] overflow-hidden shrink-0">
          ...
          <div className="p-4 text-[#b9babd] text-sm">Manage your workspace apps and integrations.</div>
        </div>
      );
      default: return <WorkspaceSidebar activeId={activeId} onSelect={handleSelectChannel} channels={channels} directMessages={directMessages} onCreateChannel={handleCreateChannel} onViewChange={setActiveView} />;
    }
  };
