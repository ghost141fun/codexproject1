...
import {
  ...
  LayoutGrid, CheckCircle2, Database, BarChart2,
  Users, Code2, Search, Plus, X, Check, Zap
} from 'lucide-react';
  ...
  return (
    <div className="w-[260px] h-full flex flex-col bg-[#19171d] border-r border-white/[0.07] overflow-hidden shrink-0">
      ...
      <div className="px-3 pt-3 pb-2 shrink-0">
        <button
          onClick={() => onStart(channels[0]?.id ?? '', channels[0]?.name ?? 'general')}
...
/* ── Huddle Hub ──────────────────────────────────────────────────────────── */
function HuddleHub({ channels, user, onStart }: {
  channels: any[]; user: any; onStart: (id: string, name: string) => void;
}) {
  return (
                ...
                <div className="flex items-start justify-between">
                  ...
                  <Avatar className="w-7 h-7 border-2 border-[#222529]">
                    ...
                    <AvatarFallback className="text-[10px] bg-[#4a154b] text-white">
                      {(user?.display_name || user?.username || user?.email || 'U').split('@')[0][0].toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                </div>
                <div>
...
function FilesView({ files, category, searchQuery, onDelete }: {
  ...
  const getFileIcon = (type: string) => {
    ...
    if (type?.includes('javascript') || type?.includes('typescript') || type?.includes('json') || type?.includes('html'))
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  {file.url && (
                    <a href={file.url} target="_blank" rel="noopener noreferrer"
                      className="h-6 w-6 flex items-center justify-center rounded hover:bg-white/10 text-[#b9babd] hover:text-white transition-colors">
                  ...
                  <button onClick={() => onDelete(file)}
                    className="h-6 w-6 flex items-center justify-center rounded hover:bg-red-500/20 text-[#b9babd] hover:text-red-400 transition-colors">
    ...
    <div className="flex-1 flex flex-col bg-[#1a1d21]">
      <div className="h-[49px] flex items-center px-4 border-b border-white/[0.07] shrink-0">
        <div className="flex items-center gap-2">
          <Avatar className="w-7 h-7 rounded-lg">
            ...
            <AvatarFallback className="rounded-lg text-xs bg-[#4a154b] text-white">
      ...
      <div className="flex-1 flex items-center justify-center flex-col gap-3 text-[#b9babd]">
        <Avatar className="w-16 h-16 rounded-2xl">
          <AvatarImage src={activeDm.avatar} />
          <AvatarFallback className="rounded-2xl text-2xl bg-[#4a154b] text-white">
            {activeDm.name?.[0]?.toUpperCase() ?? 'U'}
          </AvatarFallback>
        </Avatar>
        <p className="font-bold text-white text-[18px]">
          {activeDm.name.includes('@') ? activeDm.name.split('@')[0] : activeDm.name}
        </p>
...
export function WorkspaceClient({ user, channels: initialChannels, directMessages, files: initialFiles }: WorkspaceClientProps) {
  ...
  const handleStartHuddle = (channelId: string, channelName: string) => {
    ...
    setHuddleChannelId(channelId);
    setHuddleChannelName(channelName);
    setActiveHuddles(prev =>
      prev.find(h => h.channelId === channelId) ? prev : [...prev, { channelId, channelName }]
    );
    setActiveView('home');
  ...
  const handleLeaveHuddle = () => {
    ...
    setHuddleChannelName(null);
  };

  const handleToggleHuddle = () => {
    if (isHuddleActive) handleLeaveHuddle();
    else handleStartHuddle(activeId, activeChannel?.name ?? '');
  };
