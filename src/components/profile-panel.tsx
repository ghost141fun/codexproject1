            ...
            {showCard && (
               ...
               <div className="flex flex-col items-center py-8 gap-5">
                 <div style={{ width: 72, height: 72, borderRadius: 20, background: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                   <Shield size={32} className="text-[#a855f7] opacity-80"/>
                 </div>
                 <div className="text-center max-w-[340px]">
                   <p className="text-[16px] font-bold text-white mb-2">ID Card Not Available</p>
                   <p className="font-mono text-[12px] text-[#6b7280] leading-relaxed">Your workspace ID card hasn't been issued yet. Request one to get your official DevTalk identity.</p>
                 </div>
                 {requestSent && (
                   <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[rgba(245,158,11,0.1)] border border-[rgba(245,158,11,0.25)]">
                 ...
                 <div className="flex gap-2">
                   {!requestSent ? (
                     <button onClick={handleRequestIDCard} disabled={isRequesting} className="bg-[#7c3aed] hover:bg-[#a855f7] text-white text-[13px] font-semibold px-5 py-2.5 rounded-xl transition-all flex items-center gap-2">
                       {isRequesting ? <Loader2 size={14} className="animate-spin" /> : <Zap size={14}/>} Request ID Card
                     </button>
                   ) : (
                     <button onClick={handleCancelRequest} disabled={isRequesting} className="bg-[#1e2026] border border-[#2a2c33] text-[#6b7280] hover:text-[#e8eaf0] text-[13px] font-semibold px-5 py-2.5 rounded-xl transition-all">
                       {isRequesting && <Loader2 size={14} className="animate-spin mr-2" />} Cancel request
                     </button>
                   )}
                 </div>
               </div>
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="flex-1">
                  <div className="flex gap-2 mb-3">
                    <button onClick={() => fileInputRef.current?.click()} className="bg-[#7c3aed] hover:bg-[#a855f7] text-white text-[12px] font-semibold px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5"><Upload size={12}/> Upload phone</button>
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <div className="flex items-center">
                    ...
                    <input type="text" value={(editing ? draft : profile)[key as keyof UserProfile]} onChange={e => editing && setDraft(p => ({ ...p, [key]:e.target.value }))} readOnly={!editing} placeholder={ph}
                ...
                {Array.from({ length:52 }, (_, w) => (
                  <div key={w} className="flex flex-col gap-[3px]">
                    {Array.from({ length:7 }, (_, d) => {
                      ...
                      return <div key={d} className="w-2.5 h-2.5 rounded-[2px] hover:ring-1 hover:ring-[#7c3aed] cursor-pointer" style={{ background: getActivityColor(cell?.count ?? 0) }} title={(cell?.count ?? 0) + ' messages'}/>;
                    })}
                  </div>
                ))}
              </div>
